var systemComponents = {
	...
	'tree-node': {
      ...
      v-if="tmp[layer].tooltip != ''"
			:text="(tmp[layer].isLayer) ? (
				...
				: (tmp[layer].tooltipLocked ? tmp[layer].tooltipLocked : '达到 ' + formatWhole(tmp[layer].requires) + ' ' + tmp[layer].baseResource + ' 来解锁（你拥有 ' + formatWhole(tmp[layer].baseAmount) + ' ' + tmp[layer].baseResource + ')')
			)
			: (
	...
	'overlay-head': {
		...
		<br>
		<span v-if="canGenPoints()"  class="overlayThing">({{tmp.other.oompsMag != 0 ? format(tmp.other.oomps) + " OOM" + (tmp.other.oompsMag < 0 ? "^OOM" : tmp.other.oompsMag > 1 ? "^" + tmp.other.oompsMag : "") + "s" : formatSmall(typeof getClickSpeed === 'function' ? getClickSpeed() : 0)}}/sec)</span>
